...
rule minimap2_long:
    input:
        ...
        query = "results/preprocessed/{sample}_long_trimmed.fastq.gz" #change to right path
    output:
...
rule racon_polishing:
    input:
        reads = "results/preprocessed/{sample}_long_trimmed.fastq.gz", #change with right path
        paf = "results/polishing/{sample}/long_reads_mapping.paf",
